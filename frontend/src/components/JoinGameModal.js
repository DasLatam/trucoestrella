...
export default function JoinGameModal({ game, onClose }) {
    ...
    const handleJoin = () => {
        ...
        socket.emit('join-room', { 
            ...
            if (response.success) {
                onClose();
                // **CORRECCIÓN CLAVE: Navegar explícitamente a la sala después de unirse.**
            ...
            <div className="bg-light-bg p-8 rounded-xl shadow-2xl w-full max-w-lg border border-light-border">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className={`p-4 rounded-lg border-2 cursor-pointer ${selectedTeam === 'A' ? 'border-red-500' : 'border-gray-700'}`} onClick={() => setSelectedTeam('A')}>
                        <h3 className="font-bold text-red-500 text-lg mb-2">Equipo Truco</h3>
                    ...
                    <div className={`p-4 rounded-lg border-2 cursor-pointer ${selectedTeam === 'B' ? 'border-blue-500' : 'border-gray-700'}`} onClick={() => setSelectedTeam('B')}>
                        <h3 className="font-bold text-blue-500 text-lg mb-2">Equipo Estrella</h3>
                ...
                <div className="flex justify-end space-x-4 mt-8">
                    ...
                    <button type="button" onClick={handleJoin} className="py-2 px-6 rounded-md bg-truco-green hover:bg-opacity-90 text-white font-bold">Unirse</button>
                </div>
            </div>
        </div>
    );
